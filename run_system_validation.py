...
Completely validates frontend compilation, local SQLite states, API endpoints, and Playwright Web E2E workflows.
"""
...
def main():
    ...
    # 4. Execute Playwright E2E Web Tests
    if success:
        print_section("Step 4: Playwright E2E Web Tests")
        print("Running E2E browser tests to validate frontend UI, passcode auth, styling carousel, and checkout flows...")
        
        test_ok, stdout, stderr = run_command(["npx", "playwright", "test"], cwd=frontend_dir)
        if test_ok:
            print(f"{GREEN}✓ All E2E web tests completed successfully!{RESET}")
            print(stdout)
        else:
            print(f"{RED}❌ E2E web tests failed!{RESET}")
            print(f"{YELLOW}Playwright Output:{RESET}\n{stdout}")
            print(f"{RED}Playwright Error:{RESET}\n{stderr}")
            success = False
            
    # 5. Clean up Backend Server
    ...
    if success:
        print(f"{BOLD}{GREEN}✓ SUCCESS: All system workflows, E2E web tests, and frontend compilations are fully functional!{RESET}")
        sys.exit(0)
    else:
        print(f"{BOLD}{RED}❌ FAILURE: System E2E validation failed. Please review errors above.{RESET}")
        sys.exit(1)
